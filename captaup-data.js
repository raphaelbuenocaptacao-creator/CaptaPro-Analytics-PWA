...
async function loadCaptaUpAnnual(){
  ...
  annualPromise=(async()=>{
    ...
    if(!data||data.v!==2||!Array.isArray(data.d)||!Array.isArray(data.r)||data.r.length!==3463)throw new Error('BASE_INVALIDA_'+(data?.r?.length??0));
    if(data.from!=='2026-01-01'||data.to!=='2026-08-29')throw new Error('PERIODO_INVALIDO');
    stage('3.463 casais · base até '+br(data.to));
